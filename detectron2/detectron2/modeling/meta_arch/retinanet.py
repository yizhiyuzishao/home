...
class RetinaNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # 从主干网络获得特征图的输出形状p3p4p5p6p7
        if len(self.backbone.output_shape()) != len(self.head_in_features):
            logger.warning("[RetinaNet] Backbone produces unused features.")

        # Anchors
        self.anchor_generator = anchor_generator # 根据cfg配置的参数，生成anchor
        self.box2box_transform = box2box_transform # 把box(dx,dy,dw,dh)转化成为偏移值表示
        self.anchor_matcher = anchor_matcher # 预测出的box和ground_truth进行匹配

        self.num_classes = num_classes # 目标检测的种类数
        # Loss parameters:
    ...
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):# 网络前线传播
        """
        Args:
        ...
        """
        # batched_inputs 是一个batch_size大小的输入
    ...
    def inference(# 网络推理，如果图片大小一样，统一一个batch_size进行推理
        self,
        anchors: List[Boxes],
        ...
        for img_idx, image_size in enumerate(image_sizes):
            ...
            # 根据box_delta，集合图像大小以及anchor推断出目标box的位置（为最终结果）
...
#git clone git://github.com/open-mmlab/mmdetection.git
    def inference_single_image(
        # 如果图片大小不一样，不能统一成为一个batch_size则一张一张推理
        self,
        anchors: List[Boxes],
        box_cls: List[Tensor],
